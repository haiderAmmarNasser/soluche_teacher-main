...
class _AssignmentScreenState extends State<AssignmentScreen> {
  ...
  Widget _buildTextSubmission({required String text}) {
    return Text.rich(
      TextSpan(
        ...
        children: replaceLink(text: text).map<InlineSpan>(
          (data) {
            if (isLink(data)) {
              return TextSpan(
                text: data,
  ...
  Widget _buildAssignments(List<ReviewAssignmentsSubmission> reviewAssignment) {
    ...
    if (_currentlySelectedAssignmentFilter == allKey) {
      final List<ReviewAssignmentsSubmission> accept_assignment =
          reviewAssignment.where((e) => e.status == 1).toList();
      final List<ReviewAssignmentsSubmission> reject_assignment =
          reviewAssignment.where((e) => e.status == 2).toList();
      final List<ReviewAssignmentsSubmission> submited_assignment =
          reviewAssignment.where((e) => e.status == 0).toList();
      final List<ReviewAssignmentsSubmission> resubmitted_assignment =
          reviewAssignment.where((e) => e.status == 3).toList();

      return Column(
        children: [
          ...resubmitted_assignment.map(
    ...
    if (_currentlySelectedAssignmentFilter == acceptedKey) {
      ...
      final List<ReviewAssignmentsSubmission> accepted_assignment =
          reviewAssignment.where((e) => e.status == 1).toList();
      //
      return Column(
        children: [
          ...accepted_assignment.map(
